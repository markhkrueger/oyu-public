...
const altReadCommand = [0x8B, 0x01, 0x80, 0x33, 0x01, 0, 0, 0, 0];
...
exports.readTemperatureVP=function(callback, converter){
    const temperVendor = 3141;
    const temperProduct = 29697;
 
    if(!converter) {
     converter=exports.toDegreeCelsius;
    }
   
    var device = new HID.HID(temperVendor,temperProduct);
    if (!device) {
        callback(this, new Error("not found"), null);
        return;
    }
    const t = device.write(readCommand);
    console.log("write " + t);
    const r = device.read(function(err,response){
    console.log(`error: ${err}, resp: ${response.length}`);
      device.close();
      if(err) {
       callback.call(this,err,null); 
      } else {
...
exports.readTemperatureVP((t, e, r)=> { 
    console.log("e = "  + e);
    console.log("r = "  + r);
});